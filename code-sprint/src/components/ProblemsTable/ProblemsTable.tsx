...
function ProblemsTable() {
    ...
    <tbody className='text-white'>
        {problems.map((doc, idx) => {
            const difficultyColor = doc.difficulty === "Easy"? "text-dark-green-s" : doc.difficulty === "Medium" ? 
            ...
            return (
                <tr className={`${idx % 2 ==1? 'bg-dark-layer-1' : ''}`} key={doc.id}>
                    ...
                    <td className={'px-6 py-4'}>
                        {doc.videoId ? (<AiFillYoutube fontSize={"25"} className='cursor-pointer hover:text-red-600'
