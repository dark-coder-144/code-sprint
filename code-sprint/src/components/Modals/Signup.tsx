...
const Signup = (props: Props) => {
    ...
    const handleRegister = async (e:React.FormEvent<HTMLFormElement>)=> {
        e.preventDefault();
        if(!inputs.email || !inputs.name || !inputs.password){return alert("Please fill all fields!")}
        try{
    ...
    return (
        <form className='space-y-6 px-6 py-4' onSubmit={handleRegister}>
            <h3 className='text-xl font-medium text-white'>Register to Code-Sprint</h3>
            <div>
            ...
            <div>
                ...
                <input onChange = {handleChangeInput} type="name" name="name" id="name" className='
                border-2 outline-none sm:text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5
                bg-gray-600 border-gray-500 placeholder-gray-400 text-white
                ' placeholder='Benjamin Louis'/>
            </div>
            <div>
